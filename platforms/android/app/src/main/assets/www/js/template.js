	...
	$.each(data, function (key, val) {
		...
		switch (menu_type) {
			...
			case 1:
				...
				col += '<ons-col onclick="loadItem(' + "'" + val.cat_id + "'," + "'" + addslashes(val.category_name) + "'" + '   )" style="padding:0 1px 0 0">';
				col += '<span class="round_catmenu" style="background-image: url(' + "'" + val.photo_url + "'" + ');">';
				col += '<span class="allmenus">';
				//col+='<ons-icon class="fonts_icon" icon="md-local-dining"></ons-icon>'+val.category_name;
				if ($.isArray(val.dish_list)) {
					//col+='<span><img class="fonts_icon" src="'+val.dish_list[0]+'"></span>';
				}
				col += '<span class="catename">' + val.category_name + '</span>';
				col += '</span>';
				col += '</span>';
				col += '</ons-col>';
				...
				if (x >= 2) {
					x = 0;
					html += '<ons-list-item tappable modifier="nodivider list_column_menu" > <ons-row>';
					html += col;
...
tipColumn = function (data) {
	...
	tip_list += '<option value="0">None</option>';
	if (!empty(data)) {
		$.each(data, function (key, val) {
			selected = '';
...
displayPromo = function (data) {
	...
	if ($.isArray(data.offer)) {
		...
		$.each(data.offer, function (offer_key, offer_val) {
			...
			//html+='</ons-list-item>';
			html += '<div class="imgSec" style="background-image: url(' + "'" + offer_val.photo + "'" + ');">';
			html += '<div class="imgText">';
			html += '<span>' + Math.round(offer_val.offer_percentage) + '%</span>';
			html += '</div>';
			html += '</div>';
			html += '<p class="promoDesc">' + offer_val.offer_formatted_title + '</p>';
			html += '<p class="promoDesc2">' + offer_val.offer_description + '</p>';
		});
		//html+='</ons-list>';
		$("#tabs-1").html(html);
		html = '';
	...
	if ($.isArray(data.voucher)) {
		...
		$.each(data.voucher, function (voucher_key, voucher_val) {
			...
			//html+='</ons-list-item>';
			html += '<div class="imgSec" style="background-image: url(' + "'" + voucher_val.photo + "'" + ');">';
			html += '<div class="imgText">';
			html += '<span>' + Math.round(voucher_val.amount) + (voucher_val.voucher_type == 'percentage' ? '%' : '') + '</span>';
			html += '</div>';
			html += '</div>';
			html += '<p class="promoDesc">' + voucher_val.voucher_formatted_title + '</p>';
			html += '<p class="promoDesc2">' + voucher_val.voucher_description + '</p>';
		});
		//html+='</ons-list>';
		$("#tabs-2").html(html);
		html = '';
	...
	if (!empty(data.free_delivery)) {
		...
		html += '<ons-list modifier="list_menu">';
		html += '<ons-list-item >';
		//html+='<div class="header_bg" style="background-image: url('+ "'" + val.photo_url + "'" +')" ></div>'; 	   	   
		html += '<div class="center">' + data.free_delivery + '</div>';
		html += '</ons-list-item>';
		html += '</ons-list>';

		/*html+='<div class="imgSec" style="background-image: url('+ "'" + val.photo_url + "'" +');">';
				 
  html+='</div>';
  html+='<p class="promoDesc">Food Parlour best item ever</p>';*/
...
tabbarMenu = function () {
	...
	html += '<ons-tab page="splitter.html" label="' + t("Home") + '" icon="md-home" ></ons-tab>';
...
setFloatingCategory = function (element) {
	...
	if (!empty(data)) {
		$.each(data, function (key, val) {
			...
			if (current_page_id == "page_category") {
				html += '<ons-list-item class="cate_bg" modifier="nodivider" tappable onclick="showItemPageFloating(' + clickFormat(val.cat_id + "|" + val.category_name + "|" + "1") + ')" >';
			} else {
				html += '<ons-list-item class="cate_bg" modifier="nodivider" tappable onclick="showItemPageFloating(' + clickFormat(val.cat_id + "|" + val.category_name + "|" + "2") + ')"  >';
			}
			html += '<div class="cateDiv">';
			html += '<span>' + val.category_name + '</span>';
			html += '<span class="itemcount">&nbsp;&nbsp;' + val.item_count + '</span>';
			html += '</div>';
			html += '</ons-list-item>';
...
setCarouselGallery = function (element, data) {
	...
	var total_data = parseInt(data.length) + 0;
	html += '<ons-carousel swipeable auto-scroll overscrollable id="k_photo_list">';
	$.each(data, function (key, val) {


		if (x % 4 == 1)
			html += '<ons-carousel-item tappable modifier="nodivider gallery_list" >';
		if (x % 2 == 1)
			html += '<ons-row>';

		html += '<ons-col width="50%" onclick="FullImageView( ' + "'" + val + "'" + ' )" >';
		html += '<div class="banner">';
		...
		html += '<div class="header_bg" style="background-image: url(' + "'" + val + "'" + ')"  ></div>';
		html += '</div>';
		html += '</ons-col>';
		if (x % 2 == 0)
			html += '</ons-row>';
	...
	$.each(data, function (key, val) {
		...
		if (d % 4 == 0) {
			...
			html += '<li class="c' + di + ' ' + selected + '"><div class="circle"></div></li>';
			di++;
		}
		d++;
...
setgetMerchantInfo = function (data) {
	...
	info = '<h2 class="merchantname">' + data.merchant_name + '</h2>';
	...
	info += '<ons-col width="110px"><div class="raty-stars" data-score="' + data.ratings.ratings + '"></div></ons-col>';
	info += '<ons-col>' + data.rating_text + '</ons-col>';
	info += '</ons-row>';
	info += '</div>';

	info += '<p class="small addrDiv">' + data.address + '</p>';
...
displayDishes = function (data) {
	if (data.length <= 0) {
		return;
	}

	var html = ''; var rowColor = ''; var txtColor = '';
	var list = document.getElementById('infinite_parent_category');
	...
	$.each(data, function (key, val) {
		...
		html += '<ons-list-item modifier="longdivider chevron" class="listitem" onclick="loadMainCategory(' + "'" + val.dish_id + "'," + "'" + addslashes(val.dish_name) + "'" + '   )" style="background-color: ' + rowColor + ';">';
		html += '<div class="left">';
		//html+='<ons-icon icon="fa-coffee" class="itmeicon"></ons-icon>';
		html += '<div class="is-loading medium-loader">';
		html += '<div class="spinner"></div>';
		html += '<img class="hide" src="' + val.photo + '">';
		html += '</div>';
		html += '<img class="itmeicon" src="' + val.photo + '" />';
		html += '</div>';
		html += '<div class="center">';
		html += '<span class="itemname" style="color: ' + txtColor + ';">' + val.dish_name + '</span>';
		html += '</div>';
		html += '</ons-list-item>';
...
displayCategoriesNews = function (data) {
	var html = '';
	var list = document.getElementById('news_cat_list');

	if (data.length <= 0) {
		html = '<ons-list-item> <span class="trn">No Category Found.</span></ons-list-item>';
		var newItem = ons.createElement(html);
		list.appendChild(newItem);
		html = '';
		//return;
	...
	x = 1; xx = 1; col = ''; cat_id = 0; cat_name = ''; colorNo = 0;
	var total_data = parseInt(data.length) + 0;
	$.each(data, function (key, val) {

		col += '<ons-col onclick="loadNewsItem(' + "'" + val.ID + "'" + ', ' + "'" + val.name + "'" + '  )">';
		col += '<span class="round_cate' + (colorNo > 0 ? colorNo : '') + ' ml-10">';
		col += '<div class="imgCate">';
		col += '<img class="Cateicon" src="' + val.image + '">';
		col += '</div>';
		col += '<div class="allCate" ' + (colorNo == 1 ? 'style="color:#000"' : '') + '>';
		col += '<span class="trn">' + val.name + '</span>';
		col += '</div>';
		col += '</span>';
		col += '</ons-col>';
		...
		if (xx == 1) { cat_id = val.ID; cat_name = val.name; }
		x++;
		xx++;
		colorNo++;
	});
	if (cat_id > 0) {
		$("#cateNameH").html(cat_name);
		ajaxCall('news', 'categoryId=' + cat_id);
	}
	//dump("dish data =>"+ html); 
...
displayNews = function (data) {
	...
	if (data.length <= 0) {
		html = '<ons-list-item> <span class="trn">No Post Found.</span></ons-list-item>';
		var newItem = ons.createElement(html);
		list.appendChild(newItem);
		html = '';
		//return;
	...
	$.each(data, function (key, val) {
		...
		html += '<ons-card class="ons_card" onclick="loadNewsDetails(' + "'" + val.ID + "'" + ')">';
		html += '<ons-list-item tappable>';
		html += '<div class="left customListLeft">';
		html += '<div class="is-loading ">';
		//html +='<div class="spinner"></div>';		
		html += '<img class="hide" src="' + val.newsImage + '">';
		html += '</div>';
		html += '<img class="list-item__thumbnail rounde_image" src="' + val.newsImage + '">';
		html += '</div>';
		html += '<div class="center customList">';
		html += '<span class="list-item__title">' + val.newsTitle + '</span><span class="list-item__subtitle">' + val.newsDate + '</span>';
		html += '</div>';
		html += '</ons-list-item>';
		html += '</ons-card>';
...
displayEvents = function (data) {
	...
	if (data.length <= 0) {
		html = '<ons-list-item> <span class="trn">No Event Found.</span></ons-list-item>';
		var newItem = ons.createElement(html);
		list.appendChild(newItem);
		html = '';
		//return;
	...
	$.each(data, function (key, val) {
		...
		html += '<ons-card class="ons_card" onclick="loadEventDetails(' + "'" + val.ID + "'" + ')">';
		html += '<ons-row>';
		html += '<ons-col width="25%">';
		html += '<span class="event_date">' + val.eventDate + '</span>';
		html += '</ons-col>';
		html += '<ons-col>';
		html += '<img src="' + val.eventImage + '" style="width: 100%; height: 120px;">';
		html += '</ons-col>';
		html += '</ons-row>';

		html += '<ons-row style="margin-top: -3px;">';
		html += '<ons-col class="pl-5">';
		html += '<p class="event_title">' + val.eventsName + '</p>';
		html += '<p class="event_addr">' + val.eventAddress + '</p>';
		html += '<p class="event_time">' + val.eventStartDate + ' @' + val.eventStartTime + ' - ' + val.eventEndDate + ' @' + val.eventEndTime + '</p>';
		html += '</ons-col>';
		html += '<ons-col width="25%">';
		html += '<span class="event_price">';
		html += '<ons-icon class="fonticon" icon="fa-ticket"></ons-icon>';
		if (val.paymentType == 'Free') {
			html += 'Free';
		} else {
			html += 'BD ' + val.price[0];
		}
...
displayNewsDetails = function (data) {
	//$.each( data, function( key, val ) {
	$("#news_date").html(data.newsDate);
	$("#news_image").attr("src", data.newsImage);
	$("#news_title").html(data.newsTitle);
	$("#news_description").html(data.newsDescription);
	//}
};


displayEventDetails = function (data) {
	$.each(data, function (key, val) {
		$("#event_date").html('<ons-icon icon="fa-clock-o"></ons-icon>' + val.eventStartDate + '@' + val.eventStartTime + ' - ' + val.eventEndDate + '@' + val.eventEndTime);
		$("#event_image").attr("src", val.eventImage);
		$("#event_title").html(val.eventsName);
		$("#event_address").html('<ons-icon icon="fa-map-signs"></ons-icon>' + val.eventAddress);
		$("#event_description").html(val.eventDescription);
		var strPrice = ''; var str_event_price = '';
		if (val.paymentType == 'Free') {
			strPrice = 'Free'; str_event_price = 'Free|- |:';
		} else {
			$.each(val.ticketType, function (pkey, pval) {
				if (parseFloat(pval['option_price_t']) > 0)
					strPrice += pval['option_name_t'] + ' - BD ' + pval['option_price_t'] + '</br>';
				str_event_price += pval['option_name_t'] + '|-BD ' + pval['option_price_t'] + '|:';
			});
		}
		$("#event_price").html(strPrice);
		$("#event_id").val(val.ID);
		$("#str_event_price").val(str_event_price);
		if (val.eventLatitude != '' && val.eventLongitude != '')
			merchantLocation('#event_map', val.eventLatitude, val.eventLongitude, val.eventsName + '<br>' + val.eventAddress);
	});
};
