...
var wp_ajax_url = krms_config.WPApiUrl
...
document.addEventListener('init', function (event) {
	...
	switch (page_id) {
		...
		case "parent_page_category":
			$("#parent_page_category .app_title").html('Our Menu');
			ajaxCall('loadCategoryFilterDishes', '');
			getProfileSilent();
			//tabbar_loaded = true; 
			initImageLoaded();
			initPullHook('loadCategoryFilterDishes', 'pull_parent_category', 'category_loader', '');
			break;

		case "page_category":
			//$(".app_title").html(krms_config.DialogDefaultTitle); 
			$("#page_category .app_title").html(page.data.dish_name);
			removeStorage('infinite_category');
			//ajaxCall('loadCategory','');
			ajaxCall('loadDishCategories', 'dishId=' + page.data.dish_id);
			FillBanner();
			initImageLoaded();
			...
			initPullHook('loadDishCategories', 'pull_category', 'category_loader', 'dishId=' + page.data.dish_id);
		...
		case "payment_option":
			$(".pay_now_label").html(t("Amount to Pay") + " " + $(".cart_total_value").val());
		...
		case "dinein_forms":
			...
			$(".pay_now_label").html(t("Proceed to Payment Method"));
		...
		case "info":
			...
			processDynamicAjax("GetGallery", 'type=info', 'photo_loader', '', 1);
			//initImageLoaded();   	    
			break;
		...
		case "select_payondelivery":
			$(".pay_now_label").html(t("Finalize Order Of") + " " + $(".cart_total_value").val());
		...
		case "news":
			ajaxCall('newsCategories', '121');

			initPullHook('newsCategories', 'pull_news', 'news_loader', '121');

			break;
		case "events":
			ajaxCall('mageEvent', '');

			initPullHook('events', 'event_pull', 'event_loader', '');

			break;

		case "news_details":
			ajaxCall('news/', page.data.news_id);

			break;
		case "event_details":
			ajaxCall('mageEvent', 'eventId=' + page.data.event_id);

			break;
		case "event_register":
			//ajaxCall('mageEvent','eventId='+page.data.event_id); 
			$("#register_event_id").val(page.data.event_id);
			var arrPrice = page.data.event_price.split('|:');
			$('select[name="ticketType"]').empty().append('<option value="">Select Price</option>');
			for (i = 0; i < arrPrice.length; i++) {
				if (arrPrice[i] != '') {
					var arrPriceDetails = arrPrice[i].split('|-');
					$('select[name="ticketType"]').append('<option value="' + (arrPriceDetails[0] == 'Free' ? '0' : arrPriceDetails[0]) + '">' + arrPriceDetails[0] + ' - ' + arrPriceDetails[1] + '</option>');
				}
			}
			break;
	} /*end switch*/
...
document.addEventListener('postchange', function (event) {
	...
	if (current_page == "page_home") {
	...
	} else if (current_page == "page_category") {
		dump("carousel=>" + event.carousel.id);
		if (event.carousel.id == "k_carousel") {
			console.log('Changed to ' + event.activeIndex);
			$(".dots li").removeClass("active");
		...
		dump("carousel=>" + event.carousel.id);
		if (event.carousel.id == "k_photo_list") {
			console.log('Changed to ' + event.activeIndex);
			$(".dots li").removeClass("active");
			$(".c" + event.activeIndex).addClass("active");
			$(".info_banner_index").val(event.activeIndex);
		}
	}
...
setActiveTabber = function (index) {
	onsenNavigator = document.getElementById('onsenNavigator');
	current_page_id = onsenNavigator.topPage.id;
	switch (current_page_id) {
		case "page_home":
...
var infiniteCategory = function (done) {
	...
	var ajax_uri = ajax_url + "/loadDishCategories";
...
function ajaxCall(action, data, method) {
	var news_details_id = 0; var event_details_id = '';
	if (action == 'news' || action == 'news/' || action == 'mageEvent' || action == 'newsCategories' || action == 'mageEventRegister') {
		var ajax_uri = wp_ajax_url + "/" + action;
		if (action == 'news/') {
			ajax_uri = wp_ajax_url + "/" + action + data;
			news_details_id = data;
		}
		else if (action == 'newsCategories') {
			ajax_uri = wp_ajax_url + "/" + action + "/" + data;
		}
		else if (action == 'mageEvent') {
			event_details_id = data;
		}
	} else {
	...
	ajax_request.done(function (data) {
		...
		if (data.code == 1) {
			switch (action) {
				case "loadCategoryFilterDishes":
					destroyList("infinite_parent_category");
					displayDishes(data.details.data);
					//$("#infinite_parent_category").html( tpl ) ;
					paginate_count++;
					getCartCount();
					break;
				case "loadCategory":
				case "loadDishCategories":
				...
				case "loadPromo":
					tpl = '';
					//dump("loadPromo=>" + data.details.data.enabled );
					if (data.details.data.enabled == 2) {
						displayPromo(data.details.data);
						//dump("loadPromo tpl=>" + tpl );    
						//$(".promo_wrap").html( tpl );
					} else {
				...
				case "newsCategories":
					destroyList("news_cat_list");
					destroyList("news_list");
					displayCategoriesNews(data.detail.data);
					paginate_count++;
					break;

				case "news":
					destroyList("news_list");
					displayNews(data.detail.data);
					paginate_count++;
					break;

				case "mageEvent":
					if (event_details_id != '') {
						displayEventDetails(data.detail.data);
					} else {
						destroyList("event_list");
						displayEvents(data.detail.data);
						paginate_count++;
					}
					break;

				case "news/":
					displayNewsDetails(data.detail.data);
					break;

				case "mageEventRegister":
					showAlert(data.msg);
					break;
				default:
					showAlert(data.msg);
			...
			dialogError();
		} else if (data.code == 'no_news') {
			destroyList("news_list");
			$("#news_list").html('<ons-list-item tappable><span class="trn">' + data.message + '</span></ons-list-item>');
		} else {
			/*FAILED RESPONSE*/
			switch (action) {
				case "loadCategory":
				...
				case "news":
					$("#news_list").html('<ons-list-item tappable><span class="trn">' + data.msg + '</span></ons-list-item>');
					break;
				...
				case "newsCategories":
					$("#news_cat_list").html('<ons-list-item tappable><span class="trn">' + data.msg + '</span></ons-list-item>');
					$("#news_list").html('<ons-list-item tappable><span class="trn">' + data.msg + '</span></ons-list-item>');
...
var loadMainCategory = function (dish_id, dish_name) {
	onsenNavigator.pushPage('category.html', {
		animation: "slide",
		data: {
			"dish_id": dish_id,
			'dish_name': dish_name
...
processDynamicAjax = function (action, data, target, method, single_call) {
	...
	if (action == 'news' || action == 'mageEvent') {
		endpoint = wp_ajax_url + "/" + action;
	} else {
	...
	ajax_array[timenow].done(function (data) {
		...
		if (data.code == 1) {
			switch (action) {
				...
				case "loadCategoryFilterDishes":
					$("#parent_page_category .infinite_scroll_done").val(0);
					page_category = 1;
					destroyList('infinite_parent_category');
					displayDishes(data.details.data);
					break;
				...
				case "GetGallery":
					dump("type=> " + typeData);
					if (typeData == 'type=info') {
						if (data.details.page_action == "pull_refresh") {
							destroyList('carousel_photo_list');
							infinite_page = 0;
						}
						setCarouselGallery('carousel_photo_list', data.details.data);
						setTimeout(function () {
							initImageLoaded();
						}, 100);
					} else {
...
var loadNewsDetails = function (news_id) {
	onsenNavigator.pushPage('news_details.html', {
		animation: "slide",
		data: {
			"news_id": news_id
		}
...
var loadNewsItem = function (news_cat_id, cat_name) {
	$("#cateNameH").html(cat_name);
	ajaxCall('news', 'categoryId=' + news_cat_id);
};

var loadEventDetails = function (event_id) {
	onsenNavigator.pushPage('event_details.html', {
		animation: "slide",
		data: {
			"event_id": event_id
		}
...
var showEventRegisterPage = function () {
	onsenNavigator.pushPage('event_register.html', {
		animation: "slide",
		data: {
			"event_id": $("#event_id").val(),
			"event_price": $("#str_event_price").val()
		}
	});
};

var customerEventRegister = function () {

	$(".frm_event_register").validate({
		submitHandler: function (form) {
			var params = $(".frm_event_register").serialize();
			params += "&next_step=" + getStorage("next_step");
			ajaxCall('mageEventRegister', params, "POST");
		}
	});
	$(".frm_event_register").submit();
};
